...
fn ev(phase: &str, message: &str, percent: u8, detail: &str) -> ProgressEvent {
...
impl Orchestrator {
    pub async fn run(
        ...
        push(progress, ev("decompose", "正在解析查询意图 (调用 LLM)...", 5, ""));
        ...
        let sq_list: Vec<String> = plan.sub_queries.iter()
            ...
            .collect();
        let detail_json = serde_json::json!({ "sub_queries": sq_list }).to_string();
        ...
        push(progress, ev("cite_expand",
            "追踪引用关系 — 查找参考文献...", 65,
            &serde_json::json!({"top": candidate_pool.iter().take(3).map(|p| &p.title).collect::<Vec<_>>()}).to_string()));
        ...
        push(progress, ev("cite_done",
            &format!("引用扩展: {} → {} 篇", before_cite, candidate_pool.len()),
            68, ""));

        // Step 5: Ranking
        let batch_count = (candidate_pool.len() as f64 / 20.0).ceil() as usize;
        push(progress, ev("rank",
            &format!("AI 为 {} 篇论文打分 ({} 批)...", candidate_pool.len(), batch_count),
            70, ""));
